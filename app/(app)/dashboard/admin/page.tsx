...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('/api/dashboard/admin-kpis').then(j),
  ...
  const lifecycleOption = useMemo(() => {
    ...
    return {
      ...
      legend: { bottom: 24, itemGap: 12, padding: [6, 0, 0, 0], textStyle: { color: cssVar('--muted-foreground', fg) } },
      grid: { left: 56, right: 16, top: 24, bottom: 80, containLabel: true },
  ...
  const breachesOption = useMemo(() => {
    ...
    // Aggregate counts by status from the breach list shape returned by API
  ...
  const radarOption = useMemo(() => {
    ...
    return {
      ...
      radar: { indicator: refHealth.map((d) => ({ name: d.category ?? d.domain, max: 100 })), axisName: { color: fg } },
      series: [
        {
          type: 'radar',
  ...
  return (
    <div className="p-4">
      ...
      <p className="mt-1 text-slate-600 text-sm">Enterprise overview across operations, portfolios, and compliance.</p>
      ...
      <div className="mt-4 grid gap-3 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[200px]">
            {lifecycleOption ? (
              // @ts-ignore
              <ReactECharts option={lifecycleOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[200px]">
            {navStatusOption ? (
              // @ts-ignore
              <ReactECharts option={navStatusOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
              <div className="grid h-full place-items-center text-sm text-[color:var(--muted-foreground)]">No data</div>
            )}
          </div>
      ...
      <div className="mt-4 grid gap-3 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[200px]">
            {navTrendOption ? (
              // @ts-ignore
              <ReactECharts option={navTrendOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
              <div className="grid h-full place-items-center text-sm text-[color:var(--muted-foreground)]">No data</div>
            )}
          </div>
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[200px]">
            {positionsOption ? (
              // @ts-ignore
              <ReactECharts option={positionsOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
              <div className="grid h-full place-items-center text-sm text-[color:var(--muted-foreground)]">No data</div>
            )}
          </div>
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[200px]">
            {breachesOption ? (
              // @ts-ignore
              <ReactECharts option={breachesOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
              <div className="grid h-full place-items-center text-sm text-[color:var(--muted-foreground)]">No data</div>
            )}
          </div>
      ...
      <div className="mt-4 grid gap-3 grid-cols-1 md:grid-cols-2">
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-4">
          ...
          <div className="mt-2 h-[220px]">
            {automationOption ? (
              // @ts-ignore
              <ReactECharts option={automationOption} onChartReady={(inst: any) => inst?.resize?.()} style={{ height: '100%', width: '100%', minHeight: 1, minWidth: 1 }} />
            ) : (
              <div className="grid h-full place-items-center text-sm text-[color:var(--muted-foreground)]">No data</div>
            )}
          </div>
