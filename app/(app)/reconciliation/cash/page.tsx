...
export default function CashReconPage() {
  ...
  async function load() {
    setLoading(true);
    try { const r = await fetch('/api/recon/cash'); const j = await r.json(); setRows(Array.isArray(j) ? j : []); } finally { setLoading(false); }
  }
  useEffect(() => { load(); }, []);

  async function closeException(id: string) {
    await fetch('/api/recon/cash', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    load();
  }
  ...
  return (
    <div className="p-6">
      ...
      <p className="mt-2 text-slate-600">Bank balances vs internal ledger. Close exceptions after review.</p>
